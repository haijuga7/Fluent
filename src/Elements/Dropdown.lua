...
function Element:New(Idx, Config)
	...
	function Dropdown:BuildDropdownList()
		...
		for Idx, Value in next, Values do
            ...
            -- Modifikasi MouseButton1Up:
            Creator.AddSignal(Button.MouseButton1Up, function()
                if scrollActive then
                    -- Abaikan klik jika sedang scroll
                    MouseDownPosition = nil
                    SetBackTransparency(Selected and 0.85 or 0.89)
                    return
                end
                
                local MouseUpPosition = Vector2.new(Mouse.X, Mouse.Y)
                local TimeDiff = tick() - MouseDownTime
                
                SetBackTransparency(Selected and 0.85 or 0.89)
                
                -- Jika mouse tidak bergerak jauh dan waktu click singkat, maka ini adalah click bukan scroll
                if MouseDownPosition and (MouseUpPosition - MouseDownPosition).Magnitude < 5 and TimeDiff < 0.3 then
                    local Try = not Selected
            
                    if Dropdown:GetActiveValues() == 1 and not Try and not Config.AllowNull then
                        -- Tidak melakukan apa-apa jika tidak diizinkan null
                    else
                        if Config.Multi then
                            Selected = Try
